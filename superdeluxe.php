...
$sql= "SELECT * FROM Date where (ROOM_NO>200 AND '$start_date'<=`START_DATE` AND '$end_date'>=`END_DATE`) OR (ROOM_NO>200 AND '$start_date'>=`START_DATE` AND '$end_date'<=`END_DATE`) OR (ROOM_NO>200 AND '$start_date'<=`START_DATE` AND '$end_date'<=`END_DATE` AND '$end_date'>=`START_DATE`) OR (ROOM_NO>200 AND '$start_date'>=`START_DATE` AND '$end_date'>=`END_DATE` AND '$start_date'<=`END_DATE`)";
$sql2 = "SELECT * FROM `room` WHERE ROOM_NO>200";
$count=44;
...
$ri=$no_room;
while($no_room >0 ){
    $sql= "SELECT * from `room` where ROOM_NO>200 AND ROOM_NO NOT IN (SELECT ROOM_NO FROM Date where (ROOM_NO>200 AND '$start_date'<=`START_DATE` AND '$end_date'>=`END_DATE`) OR (ROOM_NO>200 AND '$start_date'>=`START_DATE` AND '$end_date'<=`END_DATE`) OR (ROOM_NO>200 AND '$start_date'<=`START_DATE` AND '$end_date'<=`END_DATE` AND '$end_date'>=`START_DATE`) OR (ROOM_NO>200 AND '$start_date'>=`START_DATE` AND '$end_date'>=`END_DATE` AND '$start_date'<=`END_DATE`))";
    $result= mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
$m=$row["ROOM_NO"];
$p = $row["TERRIF"];
